...
| 2026-04-13 | Fixed camera upload "Unexpected token… is not valid JSON" error. **Problem:** (1) Backend `visual_search.py` had its own strict MIME whitelist that rejected camera images even after the frontend converted them to JPEG. (2) Frontend called `res.json()` directly — if Hugging Face proxy returned an HTML error page, the JSON parser crashed. **Solution:** (1) Removed backend MIME whitelist entirely (frontend already guarantees JPEG). (2) Changed frontend to `res.text()` + safe `JSON.parse()` with try-catch, so proxy/HTML errors show a readable message instead of crashing. |
